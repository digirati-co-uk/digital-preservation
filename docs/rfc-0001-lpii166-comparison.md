...
- Tokens carry `aud = api://84c62880…` (the Preservation API), `azp` = the actual caller, and `roles = Preservation.Call` (machines) or `scp` (humans). Identity resolves via `KnownClients` off the signed `azp`; the API enforces role-or-scope (§5.3 of the RFC).
...
**Audience (`aud` claim).** The token's **intended recipient** — the *resource* the client asked the token for — not who the token is from, and not who may use it. [RFC 7519 §4.1.3](https://datatracker.ietf.org/doc/html/rfc7519#section-4.1.3): the claim "identifies the recipients that the JWT is intended for", and each principal processing the token "MUST identify itself with a value in the audience claim" — validation is the recipient asking *"was this token minted for me?"*. Three subtleties:

- *Beware the theatrical sense of the word.* An API colloquially "has an audience" of consumers, so "the API has multiple audiences" sounds like a natural way to say "the API has multiple clients" — but in JWT terms it means something entirely different: *"I accept tokens addressed to several recipients, including recipients that aren't me."* The `aud` claim names the token's recipient, never the API's clientele. This slip is load-bearing in the LPII-166 model, where it is literal rather than metaphorical: because each caller mints a token for its own registration, the values in the APIs' `ValidAudiences` lists genuinely *are* the clients — the recipient check has been repurposed as a roster of senders.
- *Multiple accepted audiences are not wrong per se — what matters is whose names they are.* A `ValidAudiences` list legitimately holds several values when they are all **aliases for the same logical recipient**: the RFC's transition (one API known as `api://a616cf42…` historically and `api://84c62880…` going forward — two names, one "me") and the v1-format variance in the next bullet are both this kind. The defect is a list containing **other principals' names**. That distinction is what separates the RFC's dual-audience Phase 0 (legitimate) from the LPII-166 union list (inverted).
...
An "audience" is also just a registration with an identifier: nothing makes a registration "an API" other than exposing a URI — which is how the Web-UI registration is the stack's audience today, and how each LPII-166 caller becomes an audience of our APIs.
