...
7. **Move the human/machine discriminator to `idtyp`.** The two chokepoints (`AuthFilterIdentifier`, `CallerResolver`) now share one predicate, `ClaimsPrincipalX.IsHumanCaller`, covering v1.0 (`upn`/`unique_name`) and v2.0 (`preferred_username`) token shapes with tests for both. That predicate is still claim-shape inference; Appendix A names the authoritative discriminator (`idtyp = app`). Once every caller's tokens are guaranteed to carry `idtyp` (it is an optional claim that must be configured on the registration), switch the predicate to it.
