...
- The human-vs-machine test is **unified**: `AuthFilterIdentifier` and `CallerResolver` share a single predicate, `ClaimsPrincipalX.IsHumanCaller`, which recognises the user-identifying claims of both v2.0 (`preferred_username`) and v1.0 (`upn`/`unique_name`, raw or .NET-mapped) delegated tokens, and deliberately ignores name-shaped claims (the filter injects a `Name` claim for machine callers, so those cannot discriminate). Without this, a v1.0-format delegated token would have been classed human by the filter but *machine* by `ResolveDepositBucket` — and could have routed a **human's** deposit into a per-caller bucket. The eventual cleaner discriminator is `idtyp` (Appendix A) — see §8 Q7.
...
- Configure the **`idtyp` optional claim** on `84c62880`'s access tokens (portal only: Token configuration → add optional claim → Access → `idtyp`), so that every **app-only** token minted *for this API* positively declares itself with `idtyp: "app"`. Microsoft calls this "the most accurate way for an API to determine if a token is an app token or an app+user token" ([optional claims reference](https://learn.microsoft.com/en-us/entra/identity-platform/optional-claims-reference)). This gives the machine half of the claim-shape inference in `IsHumanCaller` an authoritative replacement (§8 Q7); provisioning it now costs nothing and avoids a later admin round-trip. The `include_user_token` additional property — which would also mark delegated tokens with `idtyp: "user"` — is **deliberately not requested**: it can only be set by editing the registration's manifest, and manifest edits are avoided as a general rule of Entra hygiene. Delegated tokens keep being recognised by their user claims; the property is purely additive and can be applied later via Microsoft Graph with no caller impact if the positive user marker is ever needed. **Scope of effect — be precise here:** optional claims attach to tokens requested *for* the configured registration, so this marks only tokens whose audience is `84c62880` — i.e. tokens that start existing at Phase 2. Transitional `api://a616cf42…` tokens never carry it, so the code keeps the claim-shape predicate until the transitional audience is retired (Phase 4) or every caller is verified onto the new audience; only then can the predicate lean on `idtyp` (§8 Q7).
- Configure the API to accept **both** audiences.
  ...
  Remove the old `"Audience": "api://a616cf42…"` key when adding this (with both present, acceptance is the *union* of the list and the singular value — pinned by `AudienceValidationTests` — so leaving it is harmless only while its value is in the list; replace, don't accumulate). **The code form is settled (2026-09-16): none is needed.** The shape above binds through the stock `AddMicrosoftIdentityWebApi(GetSection("AzureAd"))` registration both APIs already use, proven by `AudienceValidationTests`, and deploy-then-activate falls out of the key's presence or absence in per-environment config. The guarded `PostConfigure<JwtBearerOptions>` the LPII-166 branch prototyped (and [`rfc-0001-lpii166-comparison.md`](./rfc-0001-lpii166-comparison.md) §7.1 recommended hoisting) is therefore unnecessary and was not adopted. In deployed environments these keys are ECS task-definition env vars (`AzureAd__TokenValidationParameters__ValidAudiences__0`, `__1` — lists need indexed keys); [`rfc-0001-landing-sequence.md`](./rfc-0001-landing-sequence.md) spells out each activation rung in that form. Two hardening notes:
  ...
  - A mis-shaped audience config deploys cleanly and only fails when tokens arrive. **Before shipping this change, add an integration test that validates one token per accepted audience and rejects a foreign audience.** Done: `Storage.API.Tests/Integration/AudienceValidationTests.cs` pins all three config shapes through the real `AddMicrosoftIdentityWebApi` pipeline (see §8, *Known coverage gaps*). If the config location changes (next note), re-pin that test to the new shape in the same PR.
- Ship the new `IClientDirectory` allow-list in **dual mode**: prefer the resolved `azp`, fall back to `X-Client-Identity` for callers not yet migrated. Log whenever the fallback is used.
  - **As implemented**, this dual-mode resolution lives in `AuthFilterIdentifier` — the existing post-auth chokepoint that already synthesises the machine caller's `Name` claim — rather than in `GetCallerIdentity`. It resolves `azp`/`appid` against `IClientDirectory` and injects the resolved name as that same `Name` claim, falling back to the header otherwise. This keeps `GetCallerIdentity`'s signature (and its ~21 call sites) untouched; the §5.2 form remains the eventual target. With `KnownClients` empty, no `azp` resolves, so behaviour is identical to today bar the fallback log line. One operational note: the platform's own service-to-service calls self-assert `X-Client-Identity: "api-call"` (§3.2 #7), so the fallback path is taken on essentially **every internal request** until Phase 2 completes. The filter therefore warns **once per unresolved `appId`** (subsequent requests log at `Debug`), and per-caller migration progress is observed via `/whoami` (`source` flips from `header-fallback` to `token`) rather than by warning volume. Both the warning and the once-per-appId throttle are covered by `ApiAuthorizationStackTests`.

**Phase 1 — Create per-caller registrations.**
- Create a registration per caller, each with its own credential, assigned `Preservation.Call` on the API (admin consent). Add each app ID to `KnownClients`. **Never give the shared UI registration `a616cf42…` a `KnownClients` profile with a `depositBucket`:** until Phase 2 completes, every legacy machine caller — iiif-builder, Playwright, and the platform's own service-to-service calls — presents `azp`/`appid` = `a616cf42…`, so a bucket on that profile would route all of their deposits there. (A name-only profile for it is harmless; humans are unaffected either way because `IsHumanCaller` is checked first.)
...
- **The `TokenProvider` callers (the APIs' own service-to-service identity, §3.2 #7) are config-only as of step 2 (PR #284).** The choice this bullet used to pose — extend `AccessTokenProvider`, or have the internal services authenticate *as* `84c62880` itself as a stopgap — was **decided and shipped as option (a)**: `AccessTokenProvider` now takes an optional `ResourceUri` that decouples the requested resource from the client credential (the same change §8 Q1 wants for a Storage audience), with both LPII-166 branch defects fixed on the way in — see [`rfc-0001-lpii166-comparison.md`](./rfc-0001-lpii166-comparison.md) §6. Repointing these callers is now the same shape as any other: a per-service registration plus `TokenProvider__ResourceUri = api://84c62880…` (landing sequence, rung 4). No code work remains in this phase.
...
Each phase is independently reversible until Phase 4 — and Phase 4's two behavioural steps (unknown-`azp` refusal; role-or-scope enforcement) are to be built as configuration flags, off by default ([#293](https://github.com/digirati-co-uk/digital-preservation/issues/293) — planned there, not yet in code; written any time after step 2 merges, needed only before the first environment enforces), so reversibility extends through them in practice and a tagged release is deployable to any environment at any rung. Only the final header-path deletion is one-way; per the [landing sequence](./rfc-0001-landing-sequence.md), it is a separate PR landed after the **last** environment (production) is enforcing.
...
7. **Move the human/machine discriminator to `idtyp`.** The two chokepoints (`AuthFilterIdentifier`, `CallerResolver`) now share one predicate, `ClaimsPrincipalX.IsHumanCaller`, covering v1.0 (`upn`/`unique_name`) and v2.0 (`preferred_username`) token shapes with tests for both. That predicate is still claim-shape inference; Appendix A names the authoritative discriminator, and the `idtyp` optional claim is **provisioned on `84c62880` in Phase 0** — portal only, marking app-only tokens with `idtyp: "app"`; the `include_user_token` property that would also mark user tokens is deliberately deferred as a manifest-only edit (see that phase's bullet). The end-state rule is therefore `idtyp = "app"` ⇒ machine, with user tokens recognised by their user claims as now. The switch itself must wait: optional claims attach only to tokens minted *for* the configured registration, so transitional `api://a616cf42…` tokens never carry `idtyp`. Adopt the rule once every machine token the API accepts is guaranteed to carry the claim — i.e. at Phase 4 (transitional audience retired), or earlier once all machine callers are verified onto the new audience. Until then, `IsHumanCaller` can *prefer* `idtyp` when present and fall back to claim shapes — a monotonic improvement that is safe from Phase 2 onward.
