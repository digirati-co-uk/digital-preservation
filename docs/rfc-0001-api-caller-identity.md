...
- Configure the **`idtyp` optional claim** on `84c62880`'s access tokens, with the `include_user_token` additional property, so that every token minted *for this API* positively declares whether it is app-only (`idtyp: "app"`) or on-behalf-of-a-user (`idtyp: "user"`). Microsoft calls this "the most accurate way for an API to determine if a token is an app token or an app+user token" ([optional claims reference](https://learn.microsoft.com/en-us/entra/identity-platform/optional-claims-reference)). This is the authoritative replacement for the claim-shape inference in `IsHumanCaller` (§8 Q7); provisioning it now costs nothing and avoids a later admin round-trip. **Scope of effect — be precise here:** optional claims attach to tokens requested *for* the configured registration, so this marks only tokens whose audience is `84c62880` — i.e. tokens that start existing at Phase 2. Transitional `api://a616cf42…` tokens never carry it, so the code keeps the claim-shape predicate until the transitional audience is retired (Phase 4) or every caller is verified onto the new audience; only then can the predicate switch (§8 Q7).
...
7. **Move the human/machine discriminator to `idtyp`.** The two chokepoints (`AuthFilterIdentifier`, `CallerResolver`) now share one predicate, `ClaimsPrincipalX.IsHumanCaller`, covering v1.0 (`upn`/`unique_name`) and v2.0 (`preferred_username`) token shapes with tests for both. That predicate is still claim-shape inference; Appendix A names the authoritative discriminator, and the `idtyp` optional claim (with `include_user_token`, so both `app` and `user` tokens are positively marked) is **provisioned on `84c62880` in Phase 0** — see that phase's bullet. The switch itself must wait: optional claims attach only to tokens minted *for* the configured registration, so transitional `api://a616cf42…` tokens never carry `idtyp`. Switch the predicate once every token the API accepts is guaranteed to carry it — i.e. at Phase 4 (transitional audience retired), or earlier once all callers are verified onto the new audience. Until then, `IsHumanCaller` can *prefer* `idtyp` when present and fall back to claim shapes — a monotonic improvement that is safe from Phase 2 onward.
