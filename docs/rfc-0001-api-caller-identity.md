...
- Configure the API to accept **both** audiences.
  ...
  Remove the old `"Audience": "api://a616cf42…"` key when adding this. The equivalent code-side form — Microsoft's documented approach for multiple audiences — is `services.Configure<JwtBearerOptions>(JwtBearerDefaults.AuthenticationScheme, o => o.TokenValidationParameters.ValidAudiences = …)`. **The exact code form is still open:** the LPII-166 branch prototypes a *guarded* `PostConfigure<JwtBearerOptions>` that acts only when a new `Authentication:ValidAudiences` section is present, leaving the existing singular `Audience` as the fallback — which allows deploy-then-activate instead of a config-and-code cutover. [`rfc-0001-lpii166-comparison.md`](./rfc-0001-lpii166-comparison.md) §7.1 recommends adopting that form (hoisted into `DigitalPreservation.Core` so both APIs share it) and settles where the section lives; whichever is chosen, the `Audiences`-binds-to-nothing caution above applies unchanged. Two hardening notes:
