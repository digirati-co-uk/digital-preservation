...
- **The `TokenProvider` callers (the APIs' own service-to-service identity, §3.2 #7) are *not* config-only.** `AccessTokenProvider` can only request its own `client_id` as the resource (the §8 Q1 constraint), so a new per-service registration cannot request `api://84c62880…/.default` without a code change. Choose explicitly: (a) extend `AccessTokenProvider` to decouple the requested resource from the client credential — the same change §8 Q1 wants for a Storage audience, already prototyped as the `ResourceUri` option on the LPII-166 branch (two defects to fix first — see [`rfc-0001-lpii166-comparison.md`](./rfc-0001-lpii166-comparison.md) §6); or (b) as a stopgap, have the internal services authenticate *as* `84c62880` itself (own-resource then works) — which requires assigning `Preservation.Call` to `84c62880`'s **own service principal** (the same self-assignment linchpin §3.1 describes on `a616cf42`), an admin action that must be scheduled.
