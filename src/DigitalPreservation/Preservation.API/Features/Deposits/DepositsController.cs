    ...
    ) : ControllerBase
{
    ...
    public async Task<IActionResult> GetDepositAsIIIFManifestWithToken([FromRoute] string id, [FromRoute] string token)
    {
        ...
        if (key.HasText() && key.GetSlug() == id)
        {
            ...
            {
                Response.Headers["X-IIIF-Post-Url"] = reqUrl;
    ...
    public async Task<IActionResult> PostIIIFManifestToDepositWithToken([FromRoute] string id, [FromRoute] string token)
    {
        ...
        if (key.HasText() && key.GetSlug() == id)
        {
            ...
            // This expects a IIIF Manifest that was generated by the GetDepositAsIIIFManifestWithToken action,
            // and then edited in some external tool, and POSTed back.
            
            // We are only interested in RANGES for now.
            // We are using the external IIIF editor as a Range Editor, so we don't have to build complex
            // video and image selection user interfaces in the preservation UI, beyond what we already have.
            
            // Just as ManifestBuilder turns LogicalRanges into 
            // IIIF Ranges, this needs to turn IIIF Ranges into Logical Ranges (including use of fragments)
            // and update the METS for this deposit, ultimately editing/creating one or more logical structMaps.
            // We want to be able to round-trip logical structMaps and IIIF ranges. 
            // It's possible (likely) that new ranges will be created in the external tool, and/or existing
            // ranges (identified by id) will move around, or refer to different canvases or child ranges.
            
            // We should validate that the canvases themselves are the same, and retain their original IDs.
            // We won't allow users to add, edit or remove canvases in an external tool; just ranges for now.
            
            // Implementation questions:
            // - whether to treat the incoming POSTed manifest as JSON or to deserialise it with the iiif-net library.
            // - mutating any new range IDs into our form suitable to become the ID attribute of the corresponding METS element
